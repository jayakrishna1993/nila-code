...
import { NilaCodeConsoleWebviewViewProvider } from "./NilaCodeConsoleWebviewViewProvider";
import { NilaCodeGUIWebviewViewProvider } from "./NilaCodeGUIWebviewViewProvider";
...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("nila-code.nilaCodeGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand("nila-code.nilaCodeGUIView.focus");
...
function waitForSidebarReady(
  sidebar: NilaCodeGUIWebviewViewProvider,
  timeout: number,
  interval: number,
): Promise<boolean> {
...
const getCommandsMap: (
  ...
  sidebar: NilaCodeGUIWebviewViewProvider,
  consoleView: NilaCodeConsoleWebviewViewProvider,
  ...
  return {
    "nila-code.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "nila-code.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "nila-code.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("nila-code.focusEdit", args);
    },
    "nila-code.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "nila-code.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "nila-code.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "nila-code.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "nila-code.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "nila-code.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "nila-code.focusNilaCodeInput": async () => {
    ...
    "nila-code.focusNilaCodeInputWithoutClear": async () => {
      const isContinueInputFocused = await sidebar.webviewProtocol.request(
        "isContinueInputFocused",
        undefined,
        false,
      ...
      if (isContinueInputFocused) {
        ...
        sidebar.webviewProtocol?.request(
          "focusNilaCodeInputWithoutClear",
          undefined,
        );
    ...
    "nila-code.writeDocstringForCode": async () => {
    ...
    "nila-code.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "nila-code.fixGrammar": async () => {
    ...
    "nila-code.clearConsole": async () => {
      consoleView.clearLog();
    },
    "nila-code.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");
      vscode.commands.executeCommand("workbench.action.toggleDevTools");
    },
    "nila-code.debugTerminal": async () => {
    ...
    "nila-code.newSession": () => {
      sidebar.webviewProtocol?.request("newSession", undefined);
    },

    "nila-code.shareSession": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "nila-code.viewHistory": () => {
      vscode.commands.executeCommand("nila-code.navigateTo", "/history", true);
    },
    "nila-code.focusContinueSessionId": async (
    ...
    "nila-code.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "nila-code.openConfigPage": () => {
      vscode.commands.executeCommand("nila-code.navigateTo", "/config", false);
    },
    "nila-code.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
    ...
    "nila-code.forceAutocomplete": async () => {
    ...
    "nila-code.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("nila-code.focusNilaCodeInput");
        } else if (selectedOption === "$(screen-full) Open full screen chat") {
          vscode.commands.executeCommand("nila-code.openInNewWindow");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("nila-code.navigateTo", "/config");
        }
    ...
    "nila-code.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "nila-code.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "nila-code.startLocalLemonade": () => {
      startLocalLemonade(ide);
    },
    "nila-code.installModel": async (
      modelName: string,
    ...
    "nila-code.convertConfigJsonToConfigYaml": async () => {
    ...
    "nila-code.enterEnterpriseLicenseKey": async () => {
    ...
    "nila-code.toggleNextEditEnabled": async () => {
    ...
    "nila-code.openInNewWindow": async () => {
      focusGUI();
      ...
      vscode.commands.executeCommand("nila-code.newSession");
      ...
      const sessionLoader = panel.onDidChangeViewState(() => {
        ...
        if (sessionId) {
          vscode.commands.executeCommand(
            "nila-code.focusContinueSessionId",
            sessionId,
          );
        }
        panel.reveal();
