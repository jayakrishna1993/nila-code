...
const CONTINUE_GLOBAL_DIR = (() => {
  const configPath =
    process.env.NILA_CODE_GLOBAL_DIR ?? process.env.CONTINUE_GLOBAL_DIR;
  if (configPath) {
  ...
  return path.join(os.homedir(), ".nila-code");
})();
...
export function getContinueGlobalPath(): string {
  // This is ~/.nila-code on mac/linux
...
export function getConfigTsPath(): string {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    fs.writeFileSync(
      packageJsonPath,
      JSON.stringify({
        name: "nila-code-config",
        version: "1.0.0",
        description: "My NILA_Code Configuration",
        main: "config.js",
      }),
    );
  }
...
export function getContinueRcPath(): string {
  ...
  const continuercPath = path.join(getContinueGlobalPath(), ".nila-coderc.json");
